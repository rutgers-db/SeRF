...
int exp(string dataset, int data_size, string dataset_path, string method,
  ...
  string version = "scalability-halfbound";
  if (is_amarel) {
  ...
  for (float scale : scale_list) {
    ...
    data_sample.generateHalfBoundedQueriesAndGroundtruth();
    ...
    for (unsigned index_k : index_k_list) {
      for (unsigned ef_construction : ef_construction_list) {
        ...
        i_params.K = index_k;
        {
          ...
          halfrangeindex::HalfBoundedIndexUpdate<float> index(&ss,
                                                              &data_sample);
          ...
          cout << "Total # of Neighbors: " << index.index_info->nodes_amount
               << endl;
          execute_queries_in_range_search(index, search_info, data_sample,
