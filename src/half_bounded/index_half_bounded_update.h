...
class HalfBoundedHNSWUpdate : public HierarchicalNSW<float> {
 public:
  HalfBoundedHNSWUpdate(const BaseIndex::IndexParams &index_params,
  ...
  void getNeighborsByHeuristic2RecordPruned(
      ...
      const size_t M, vector<NeighborLifeCycle<dist_t>> *back_nns,
    ...
    while (queue_closest.size()) {
      ...
      if (good) {
        ...
        // Pruned Node, ADD to range_nns
        int external_nn = curent_pair.second;
        if (external_nn != end_pos_id) {
          NeighborLifeCycle<dist_t> pruned_nn(external_nn, dist_to_query,
                                              external_nn, end_pos_id);
          back_nns->emplace_back(pruned_nn);
        }
      }
    }
    // More nodes Not visited
    while (queue_closest.size()) {
      std::pair<dist_t, tableint> curent_pair = queue_closest.top();
      int external_nn = curent_pair.second;
      queue_closest.pop();
      ...
      if (external_nn != end_pos_id) {
        NeighborLifeCycle<dist_t> pruned_nn(external_nn, -curent_pair.first,
                                            external_nn, end_pos_id);
        back_nns->emplace_back(pruned_nn);
      }
    }
...
template <typename dist_t>
class HalfBoundedIndexUpdate : public BaseIndex {
 public:
  vector<vector<NeighborLifeCycle<dist_t>>> indexed_arr;

  HalfBoundedIndexUpdate(
      delta_index_hnsw_full_reverse::SpaceInterface<dist_t> *s,
      const DataWrapper *data)
      : BaseIndex(data) {
    fstdistfunc_ = s->get_dist_func();
    dist_func_param_ = s->get_dist_func_param();
    index_info = new IndexInfo();
    index_info->index_version_type = "HalfBoundedIndexUpdate";
  }
  ...
  void buildIndex(const IndexParams *index_params) override {
    ...
    HalfBoundedHNSWUpdate<float> hnsw(
    ...
    for (size_t i = 0; i < data_wrapper->data_size; ++i) {
      ...
      for (size_t j = 0; j < link_list_count; j++) {
        int node_id = data[j];
        NeighborLifeCycle<dist_t> nn(node_id, 0, node_id, node_id);
  ...
  vector<int> rangeFilteringSearchOutBound(
    ...
    {
      int lbound = query_bound.first;
      int interval = (query_bound.second - lbound) / 10;
